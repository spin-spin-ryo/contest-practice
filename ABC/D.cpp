...
bool dfs(int p, int q, vector<int> &row_A, vector<int> &row_B, vector<int> &row_C, vector<int> &col_A, vector<int> &col_B, vector<int> &col_C, int &N, string &R, string &C){
    // check row
    bool exist = false;
    // cout << p << " " << q << endl;
    if (p == N){
    ...
    if (row.size() == 3 & col.size() == 3){
        if (R[p] == C[q]){
            if (R[p] == 'A'){
                col_A[q] = p;
                row_A[p] = q;
                exist |= dfs(p + ((q+1)%N == 0),(q+1)%N,row_A,row_B,row_C,col_A,col_B,col_C,N,R,C);
                col_A[q] = -1;
                row_A[p] = -1;
            }
...
int main(){
    ...
    if (!dfs(0,0,row_A,row_B,row_C,col_A,col_B,col_C,N,R,C)){
        cout << "No" << endl;
    }
}