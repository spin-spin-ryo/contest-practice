...
int main(){
    ...
    for (ll k=0;k<N;k++){
        ...
        keys.add(index+1,1);
        if (keys.sum(index+1,index+1)==0){
            values_BIT.add(index+1,A[k]);
        }
        
        sub_sum += keys.sum(index)*A[k]%mod-values_BIT.sum(index) + values_BIT.sum(index +2 , B.size())%mod - keys.sum(index +2 , B.size())*A[k]%mod;
        sub_sum %= mod;
        ll ans = 0;
        ans += sum_A*inv(k+1)%mod + sub_sum*inv((k+1)*(k+1))%mod;
        ans %= mod;
        if (ans<0){
            ans += mod;
            ans %= mod;
