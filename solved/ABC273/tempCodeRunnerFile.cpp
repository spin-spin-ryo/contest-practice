...
int main(){
    ...
    for (ll i=0;i<Q;i++){
        ...
        if (query == "LOAD"){
            if (write_index != -1){
                if (need_index[write_index].size() < indexes.size()) need_index[write_index] = indexes;
            }
            indexes.clear();
    ...
    for (ll i=0;i<Q;i++){
        ...
        if (query == "LOAD"){
            ...
            if (itr!= need_vectors.end()) values = need_vectors[X[i]];
        }

        if (query == "SAVE"){
            indexes.clear();
            indexes = need_index[i+1];
